...
__kernel void HelloWorld(__global int* argument1, __global int* argument2, __global int* output){
	...
	printf("Hi from work item: %-3d work group:%-3d Arg1: %-3d Arg2: %-3d Output: %-3d \n", workItemNum, workGroupNum, arg1, arg2, output);
	/*
	Expected Output:
	...
	*/
	...
	if(localGroupID == 0){ //this determines if it is the first item in a work group
		for(int x=0; x<4; x++){	//this iterates through the 4 items in each work group
			groupValue = groupValue + (workItemNum+x)*arg1 + arg2; //this gets the total value for the work group
		}
		printf("groupValue: %-3d Work item: %-3d Work group: %-3d \n",groupValue,workItemNum,workGroupNum);
	}      
	barrier(CLK_LOCAL_MEM_FENCE);	//this stops the work until all the work groups are at the same position
